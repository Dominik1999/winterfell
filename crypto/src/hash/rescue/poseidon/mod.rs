...
/// Implementation of [Hasher] trait for Poseidon

pub struct Poseidon64();

impl Hasher for Poseidon64 {
    ...
    fn hash(bytes: &[u8]) -> Self::Digest {
        ...
        for chunk in bytes.chunks(7) {
            ...
            // state; if the rate is filled up, apply the permutation and start absorbing
        ...
        // the number of elements is not a multiple of RATE_WIDTH), apply the permutation.
...
impl ElementHasher for Poseidon64 {
...
impl Poseidon64 {
    ...
    pub const EIGHT: BaseElement = BaseElement::from_canonical_u64(MDS_MATRIX_DIAG[0]);
    ...
    pub fn apply_permutation(state: &mut [BaseElement; STATE_WIDTH]) {
        ...
        Self::full_rounds(state, &mut round_ctr);
    ...
    fn apply_mds(state: &mut [BaseElement; STATE_WIDTH]) {
        ...
        for r in 0..STATE_WIDTH {
            let s = state[r].to_noncanonical_u64();
            state_h[r] = s >> 32;
        ...
        for r in 0..STATE_WIDTH {
            ...
            let out = res.wrapping_add(0u32.wrapping_sub(over as u32) as u64);
            //assert!(out < BaseElement::MODULUS, "{:?} is not canonical", out);
            result[r] = BaseElement::from_canonical_u64(out);
        }
        result[0] += state[0] * Self::EIGHT;
        *state = result;
    }
    ...
    #[inline(always)]
    fn partial_first_constant_layer(state: &mut [BaseElement; STATE_WIDTH]) {
        for i in 0..12 {
            if i < STATE_WIDTH {
                state[i] += BaseElement::from_canonical_u64(FAST_PARTIAL_FIRST_ROUND_CONSTANT[i]);
            }
        }
    }

    #[inline(always)]
    fn mds_partial_layer_init(state: &[BaseElement; STATE_WIDTH]) -> [BaseElement; STATE_WIDTH] {
        let mut result = [BaseElement::ZERO; STATE_WIDTH];
            ...
            if r < STATE_WIDTH {
                for c in 1..12 {
                    if c < STATE_WIDTH {
                        ...
                        // friendly.
                        let t = BaseElement::from_canonical_u64(
    ...
    fn mds_partial_layer_fast(
        ...
        for i in 1..12 {
            if i < STATE_WIDTH {
                let t = BaseElement::from_canonical_u64(FAST_PARTIAL_ROUND_VS[r][i - 1]);
    ...
    #[inline]
    fn partial_rounds(state: &mut [BaseElement; STATE_WIDTH], round_ctr: &mut usize) {
    ...
    #[inline]
    fn full_rounds(state: &mut [BaseElement; STATE_WIDTH], round_ctr: &mut usize) {
    ...
    #[inline(always)]
    fn constant_layer(state: &mut [BaseElement; STATE_WIDTH], round_ctr: usize) {
        for i in 0..12 {
            if i < STATE_WIDTH {
                let round_constant = ALL_ROUND_CONSTANTS[i + STATE_WIDTH * round_ctr];
                unsafe {
...
#[inline(always)]
fn reduce_u160((n_lo, n_hi): (u128, u32)) -> BaseElement {
...
#[rustfmt::skip]
pub const ALL_ROUND_CONSTANTS: [u64; STATE_WIDTH * N_ROUNDS]  = [
